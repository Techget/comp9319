...
int occWithIndex(char c, int index){
	if (index >= THRESHOLD) {
		...
		char ch;
		int i = 0;
		// cout<<"bwt file offset ftell result:"<<ftell(bwt_file)<<" amount_need_brute_force: "<<amount_need_brute_force<<"\n";
		...
		// cout<<"read_arr[0]: "<<read_arr[0]<<" brute force count rank: "<<rank-read_arr[0]<<"\n";
		// cout<<"after while loop ftell result: "<<ftell(bwt_file)<<"\n";
		// cout<<"occWithIndex output counter: "<<rank<<"\n";
		return rank;
	} else {
