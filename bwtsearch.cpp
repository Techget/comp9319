...
*/
void recoverTexts(int (*occFunc)(char,int), int (*inverseOccFunc)(char,int), int arg_count, char* arg_v[]){
	// cout<<LAST_ptr - FIRST_ptr<<"\n";
	if(arg_count == 4 || LAST_ptr - FIRST_ptr <= 50){
		int i;
		...
		// cout<<"come into else in recoverTexts\n";
		// goes into here, means argc > 4
		std::map<int, int> intersection_map;
		std::map<int, int> print_times;
		std::map<int, int> print_start_index;
		// get the intersection of 
		int i;
		for (i=FIRST_ptr; i<=LAST_ptr; i++) {
			string recordId = "";
			int meet_close_bracket_flag = 0;

			fseek(bwt_file, i, SEEK_SET);
			char prefix_char = fgetc(bwt_file);

			if (prefix_char == ']') {
				meet_close_bracket_flag = 1;
			}

			int index = c_array_map[prefix_char] + (*occFunc)(prefix_char, i)-1;	
			while(true){
				fseek(bwt_file, index, SEEK_SET);
				prefix_char = fgetc(bwt_file);
				...
				if(meet_close_bracket_flag == 1){
					recordId = prefix_char + recordId;
				}
			...
			int record_id_int = atoi(recordId.c_str());
	        print_start_index[record_id_int] = index;

	        if (intersection_map.find(record_id_int) == intersection_map.end()) {
	        	// not find the key
	        	intersection_map[record_id_int] = 1;
	        } 

	        if (print_times.find(record_id_int) == print_times.end()) {
	        	print_times[record_id_int] = 1;
	        } else {
	        	print_times[record_id_int]++;
	        }
		}
		...
		for(i = 4; i<arg_count; i++){
			FIRST_ptr = -1;
			LAST_ptr = -1;
			string pattern = arg_v[i];
			int pattern_exist_flag = 0;
			if (pattern.length() == 1) {
				// directly return the range we can get from c_array_map
				FIRST_ptr = c_array_map[pattern.at(0)];
				int next_char = nextKey(pattern.at(0));
				...
				if(FIRST_ptr<=LAST_ptr && FIRST_ptr != -1 && LAST_ptr != -1){
					pattern_exist_flag = 1;
				}
			} else if (idx_file_exist_flag) {
				// can assume that if it is exist, it should be opened already
				bwtBackwardSearch(occWithIndex, pattern);
			...
			if(pattern_exist_flag == 1){
				int ind;
				for (ind=FIRST_ptr; ind<=LAST_ptr; ind++) {
					string recordId = "";
					int meet_close_bracket_flag = 0;

					fseek(bwt_file, ind, SEEK_SET);
					char prefix_char = fgetc(bwt_file);
					...
					int index = c_array_map[prefix_char] + (*occFunc)(prefix_char, ind)-1;	
					while(true){
						fseek(bwt_file, index, SEEK_SET);
						prefix_char = fgetc(bwt_file);
					...
					if(meet_close_bracket_flag == 1){
						int record_id_int = atoi(recordId.c_str());
						if (intersection_map.find(record_id_int) != intersection_map.end() 
							&& intersection_map[record_id_int] > 0) {
							// key exist
							// cout<<"intersection_map[record_id_int]: "<<intersection_map[record_id_int]<<"\n";
							// cout<<"i - 3: "<<(i-3)<<"\n";
				        	if (intersection_map[record_id_int] == (i - 3)) {
					        	// cout<<"come in here!!!\n";
					        	intersection_map[record_id_int] = i - 3 + 1;
					        }
				        }
				    }
				}
		...
		// find intersected recordId and recover the string
		map<int, int>::iterator iter;
		for(iter = intersection_map.begin(); iter != intersection_map.end(); iter++ ){
			// 5 - 3 == 2
			// 6 - 3 == 3
			// cout<<"iter->second:"<<iter->second<<"arg_count - 3:"<<(arg_count - 3)<<"\n";
			if(iter->second == arg_count - 3){
				// recover the string forwardly
				string output_str = "";
				//forward search to the fisrt [
				int index = print_start_index[iter->first];
				char concat_char;
				int curr_char_rank;
				// stop when you meet open bracket the second time
				int meet_open_bracket_flag = 0;
				// int meet_close_bracket_flag = 0;
				while(true){
					map<char, int>::iterator it = c_array_map.begin();
					char prev_key = it->first;
					...
					// skip, since it's inside []
					// if (concat_char == ']') {
					// 	meet_close_bracket_flag++;
					// }
					// end forward search;
					if (concat_char == '[') {
						meet_open_bracket_flag++;
					}

					if (meet_open_bracket_flag ==  2){
						break;
					}
				...
				// according to print_times, print it out 
				int i;
				for(i = 0; i<print_times[iter->first]; i++){
					cout<<output_str<<"\n";
				}
			}
		}
