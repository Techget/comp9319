...
int main(int argc, char* argv[]) {
	...
	if( third_arg == "-c") {
		...
		if (correlation_parameter == 1.0) {
			...
			concept_search(search_terms, inverted_index, 0.00001, idx_folder);
		} else {
			concept_search(search_terms, inverted_index, correlation_parameter, idx_folder);
		}
	} else {
...
void concept_search(std::vector<std::string> & search_terms, 
	...
	std::map<std::string, std::vector<std::string> > word_and_its_synonym;

	std::ifstream synonym_file("synonyms", std::fstream::in);
			...
			if (str_temp.find(*search_term_itr) != std::string::npos) {
        		...
        		std::regex_token_iterator<std::string::iterator> reg_tok_itr (str_temp.begin(), str_temp.end(), reg);
  				...
  				while (reg_tok_itr != rend) {
  					word_and_its_synonym[*search_term_itr].push_back(*reg_tok_itr++);
		  		}
				break;
			}
		}
	...
	std::map<std::string, std::vector<std::string> >::iterator map_itr;
	// map_itr->first is the search_term
	// map_itr->second is the vector of synonyms of search term
	for (map_itr = word_and_its_synonym.begin();
		...
		/** synonym search **/
		std::vector<std::string>::iterator synonym_itr;
