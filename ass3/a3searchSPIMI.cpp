...
int key_word_search(std::vector<std::string>& search_terms, 
	std::map< std::string, std::map<std::string, int> > & inverted_index,
	std::string & idx_folder);
int concept_search(std::vector<std::string>& search_terms, 
	std::map< std::string, std::map<std::string, int> > & inverted_index,
	float correlation_parameter,
	std::string & idx_folder);
...
int main(int argc, char* argv[]) {
	...
	if( third_arg == "-c") {
		...
		if (correlation_parameter == 1.0) {
			suc_flag = key_word_search(search_terms, inverted_index, idx_folder);
		} else if (correlation_parameter == 0){
			// use an very small weight instead
			suc_flag = concept_search(search_terms, inverted_index, 0.00001, idx_folder);
		} else {
			suc_flag = concept_search(search_terms, inverted_index, correlation_parameter, idx_folder);
		}
...
// specifial for concept search
int concept_search(std::vector<std::string> & search_terms, 
	std::map< std::string, std::map<std::string, int> > & inverted_index,
	float correlation_parameter,
	std::string & idx_folder) {
	// ##### first part 1
	// find corresponding word and its synonym, and their appearance  
	// file list and scores. #size(search_terms) lists there will be
	std::map<std::string, std::set<std::string> > word_and_its_synonym;

	std::ifstream synonym_file("synonym_dict.txt", std::fstream::in);

	std::vector<std::string>::iterator search_term_itr;
	for (search_term_itr = search_terms.begin(); search_term_itr != search_terms.end(); ++search_term_itr) {
		std::string str_temp;
		while (std::getline(synonym_file, str_temp)) {
			if (str_temp.find(" " + *search_term_itr + " ") != std::string::npos) {
        		...
        		std::regex_token_iterator<std::string::iterator> reg_tok_itr (str_temp.begin(), str_temp.end(), reg);
  				...
  				while (reg_tok_itr != rend) {
  					word_and_its_synonym[*search_term_itr].insert(*reg_tok_itr++);
		  		}
				break;
			}
		}
		synonym_file.clear();
		synonym_file.seekg(0, std::ios::beg);
	}

	synonym_file.close();

	// ##### second part
	// ##### loop through word_and_its_synonym to fill in inverted_index
	// ##### read index_file
	...
	std::map<std::string, std::set<std::string> >::iterator map_itr;
	// map_itr->first is the search_term
	// map_itr->second is the set of synonyms of search term
	for (map_itr = word_and_its_synonym.begin();
		map_itr != word_and_its_synonym.end();
		map_itr++) {
		/** keyword search for search term **/
		char start_with = map_itr->first.at(0);
		std::string str_temp;
		while (std::getline(in_file, str_temp)) {
			...
			if (str_temp.find(map_itr->first+":") == 0) {
		...
		/** synonym search **/
		std::set<std::string>::iterator synonym_itr;
		for (synonym_itr = map_itr->second.begin(); 
			synonym_itr != map_itr->second.end(); 
			++synonym_itr) {

			start_with = (*synonym_itr).at(0);
			std::string str_temp;
			while (std::getline(in_file, str_temp)) {
				// used to speed up search
				if (str_temp.at(0) != start_with) {
				...
				// start with this stemmed word
				if (str_temp.find(*synonym_itr+":") == 0) {
					std::regex reg ("[^,:|]+"); // regex to extract words
			        std::regex_token_iterator<std::string::iterator> reg_tok_itr (str_temp.begin(), str_temp.end(), reg);
			  		std::regex_token_iterator<std::string::iterator> rend;

			  		// skip the synonym stemmed word
			  		reg_tok_itr++;

			  		while (reg_tok_itr != rend) {
			  			std::string filename = *reg_tok_itr++;
			  			std::string occurence = *reg_tok_itr++;
			  			if (inverted_index.find(map_itr->first) != inverted_index.end()) {
			  				if (inverted_index[map_itr->first].find(filename) != inverted_index[map_itr->first].end()) {
			  					inverted_index[map_itr->first][filename] += std::stoi(occurence) * correlation_parameter;
			  				} else {
			  					inverted_index[map_itr->first][filename] = std::stoi(occurence) * correlation_parameter;
			  				}
			  			} else {
