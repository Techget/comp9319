...
int inverseOccWithIndex(char c, int rank){
	...
	int left_block = 0;
	int right_block = bwt_file_size / THRESHOLD - 1;
	int middle_block = -1;
	
	if(left_block == right_block){
		return inverseOccWithoutIndex(c, rank);
	} else if (right_block - left_block < 10) {
		cout<<"$$$$$$$$$$\n";
		int rank_prev;
		while(rank_temp < rank){
			// cout<<"here in original while loop\n";
			rank_prev = rank_temp;
		...
		// initialize left/right_value first
		int left_value = rank_temp;
		curr_idx_file_offset = right_block * 4 * CHAR_AMOUNT + char_offset;
		fseek(idx_file, curr_idx_file_offset, SEEK_SET);
		fread(read_arr, sizeof(int), 1, idx_file);
		int right_value = read_arr[0]; 

		while (true) {
			if(right_block - left_block <= 2){
				break;
			}
			middle_block = (left_block + right_block) / 2;
			int offset = middle_block * 4 * CHAR_AMOUNT + char_offset;
			fseek(idx_file, offset, SEEK_SET);
			fread(read_arr, sizeof(int), 1, idx_file);
			int middle_value = read_arr[0];
			if (middle_value > rank) {
				right_block = middle_block;
				right_value = middle_value;
			} else if(middle_value < rank) {
				left_block = middle_block;
				left_value = middle_value;
			} else {
				// equal
				left_block = middle_block - 1;
				curr_idx_file_offset = left_block * 4 * CHAR_AMOUNT + char_offset;
				fseek(idx_file, curr_idx_file_offset, SEEK_SET);
				fread(read_arr, sizeof(int), 1, idx_file);
				left_value = read_arr[0];
				break;
			}
		}

		number_of_block = left_block + 1;
		rank_temp = left_value;
	}
